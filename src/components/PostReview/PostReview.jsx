...
function PostReview({ onClose, apiUrl }) {
  ...
  useEffect(() => {
    fetch(`https://voz-estudiantil-backend-production.up.railway.app/universities`)
  ...
  const handleUniversityChange = (selectedUniversityId) => {
    ...
    fetch(`https://voz-estudiantil-backend-production.up.railway.app/campuses?university_id=${selectedUniversityId}`)
      .then((response) => {
        if (!response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`https://voz-estudiantil-backend-production.up.railway.app/reviews`, {
        method: "POST",
        body: formData,
      });
